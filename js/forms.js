 ...
 function handleFileUpload(inputId) {
    ...
    input.addEventListener('change', function() {
        if (this.files.length > 0) {
            // Adiciona estilos de sucesso
            container.classList.add('file-uploaded');
            icon.classList.remove('bi-cloud-upload', 'text-secondary');
            icon.classList.add('bi-check-circle', 'upload-success');
            label.innerHTML = 'Arquivo anexado!<br><small class="text-muted">' + this.files[0].name + '</small>';
        } else {
            // Remove estilos de sucesso
            container.classList.remove('file-uploaded');
            icon.classList.add('bi-cloud-upload', 'text-secondary');
            icon.classList.remove('bi-check-circle', 'upload-success');
            label.innerHTML = 'Clique aqui para<br>selecionar o arquivo';
        }
    });
}

// Inicializa para ambos os campos
handleFileUpload('comprovante');
handleFileUpload('documento');